...
export async function generatePromoThumbnailAction(): Promise<GeneratePromoThumbnailOutput | { error: string }> {
  try {
    ...
    return { error: 'Gagal membuat gambar promosi.' };
  }
}

export async function generateAffiliatePromoAction(input: GenerateAffiliatePromoInput): Promise<GenerateAffiliatePromoOutput | { error: string }> {
  ...
  try {
    ...
    return { error: 'Gagal membuat teks promosi.' };
  }
}
