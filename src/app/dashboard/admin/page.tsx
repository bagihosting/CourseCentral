...
import { User, Pencil, Loader2, Camera } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    let newAvatarUrl = selectedUser.avatarUrl;
  ...
  return (
    <>
                ...
                {users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-right">
                       <Button variant="outline" size="sm" onClick={() => handleEditClick(user)} disabled={user.id === 'admin'}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
                <DialogTitle>Ubah Pengguna: {selectedUser?.name}</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Biarkan kosong jika tidak ingin mengubah" />
                </div>
