...
import { Sparkles, Lock, FileText, Bot, ArrowLeft, Plug, Megaphone, Mail } from 'lucide-react';
...
function ProFeatures() {
  const [activeApp, setActiveApp] = useState<'blogger' | 'skripsi' | 'wordpress' | 'google-ads' | 'digital-invitation' | null>(null);

  const handleAppSelect = (app: 'blogger' | 'skripsi' | 'wordpress' | 'google-ads' | 'digital-invitation') => {
    setActiveApp(app);
  };
      ...
      {activeApp === null ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
          {apps.map((app) => (
            <Card
              key={app.id}
              onClick={() => handleAppSelect(app.id as 'blogger' | 'skripsi' | 'wordpress' | 'google-ads' | 'digital-invitation')}
